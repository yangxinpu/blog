...
进入 A → 设置 value = 1 进入 B → 设置 value = 2 渲染 Child（读到 2） 离开 B →
恢复 value = 1 离开 A → 恢复默认值
```
    ...
    <>
      ...
      <Routes>
        ...
        <Route
          path="/about"
          element={
            <Profiler id="about" onRender={about}>
              <About />
            </Profiler>
          }
        />
...
class Parent extends React.Component {
  ...
  render() {
    return (
      <div>
        //点击按钮时，父组件因 count 变化重新渲染，但 UserCard 接收的
        props（name 和 age）没有变化，因此 UserCard 不会重新渲染
