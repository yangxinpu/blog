...
const state = reactive({ // 创建一个 reactive 响应式对象
...
watch(
  ...
  (newValue, oldValue, onCleanup) => { // callback：数据变化时触发；第三个参数用于注册清理函数
    ...
    onCleanup(() => { // 注册清理函数，在下一次 watch 执行前触发
...
```
<button v-on:click="print('aaaaaa',$event)">按钮</button>//$event为事件对象的占位符

<button @click.prevent="print">按钮</button>

<input type="text" id="root" @keyup.enter.y="print">//同时按下enter和y才触发事件
```
...
//数字范围
    <ul>
        <li v-for="k in n" :key="k">{{k}}</li>//展示10个内容为从1到10的li
    </ul>
</div>
```
...
<template>
    <component :is="which" :key="current"></component>//渲染哪个组件根据which来决定
...
<template>
  <div class="app">
    <input type="text" v-my-focus:100="a" />//binding.arg为"100"，binding.value为a;
  </div>
</template>
```
...
nextTick(() => {//如果把nextTick放在这里，那它只会执行一次输出0
  console.log('Updated value:', a.value);
});
</script>
...
<template>
  <div>
    <Son >
      <template v-slot:s1>//插入名为s1的slot标签的内容
        <h1>s1插槽</h1>
      </template>
      
      <template #s2>//插入名为s2的slot标签的内容
        <h1>s2插槽</h1>
      </template>
    </Son>
  </div>
