...
使用的构建与打包工具�?Webpack，同时集成了完整的前端工程化工具链，包括Babel，Postcss，Webpack-Loader等；
...
Vite�?Vue CLI 5+已内置了 CSS 预编译器的编译能力，只需安装对应预编译器依赖，即可直接使用；需要在`style`标签中指定对应的lang�?
...
�?`script` 标签中直�?`import CSS` 时，无论在哪个模块中导入，CSS 的样式规则都会作为全局样式生效，不存在作用域隔离；构建工具�?Vite / Webpack）会在构建过程中收集所有被导入的全局样式并进行合并处理，最终通常打包为一个全局 CSS 文件（如果存在动态导入或代码分割，则可能生成多个按需加载的全局 CSS 文件）；

- 开发环境：`import './style.css'` 时，Vite/Webpack 会将 CSS 内容转换为字符串并封装为 JS 模块，运行时�?JS 创建 `<style>` 标签并插入到 `document.head` 中，从而使样式生效，同时这种方式也便于实现 HMR�?
- 生产环境：`import './style.css'` 时，构建工具会在构建阶段收集所�?CSS 资源并进行合并与优化，生成独立的 `style.css` 文件，并�?HTML 中通过 `<link>` 标签引入�?CSS 文件�?
...
- 如果同时使用 `scoped`，无论是外部CSS，还是当�?`<style>` 中编写的样式，都会在编译阶段被添加作用域属性选择器（�?`data-v-xxxx`），从而只作用于当前组件的 DOM
- 如果多个组件都通过 `<style src="./assets/css/loading.css" scoped></style>` 引入同一�?CSS 文件，由�?`scoped` 的作用域是按组件生成唯一标识实现的，构建时每个组件都会生成不同的作用域属性，因此同一�?CSS 文件会被编译多次并生成多份带不同作用域的样式规则，使各组件样式互不影响，但也会造成一定的样式重复�?

```
<style src="./assets/css/loading.css" scoped></style>//引入的外部文�?
<style scoped>
    //自己的css文件
</style>
```
...
当组件的 `<style>` 标签添加 `scoped` 属性时（如果不使用则会被当做全局样式处理），Vue 在编译阶段会为该组件生成一�?唯一的作用域标识（如 `data-v-hash`）随�?Vue 会做两件事：

1. 给组件模板中的所�?DOM 元素添加该属性（�?`data-v-hash`）�?
2. �?`<style>` 中的每个 CSS 选择器转换为带有该属性的属性选择器（�?`.title` �?`.title[data-v-hash]`）�?

这样样式规则只会匹配带有该属性的元素，从而实�?组件级的样式隔离�?

当组件内部包含子组件时：

- 子组件根元素会同时携带父组件的作用域属�?�?子组件自己的作用域属性�?
...
其实现原理是在编译阶段对选择器进行转换：Vue 在处理包�?`:deep()` 的选择器时，会只给 `:deep()` 前面的部分添加作用域属性，而不会给 `:deep()` 内部的选择器添加作用域限制
...
//编译后，这样就能影响到Home�?home�?
.app[data-v-7a7a37b1] .home {
    color: blue;
}
[data-v-7a7a37b1] .home {
    color: blue;
}
```
...
当使�?对象语法 时，对象的键表示类名，值表示一个布尔表达式：当值为 `true` 时会添加该类名，当值为 `false` 时则不会添加该类�?

可以和静态类名一起使用：Vue 会自�?合并静态类名和动态类�?
...
#### CSS中的v-bind()
 ...
 `<style>`标签支持使用 `v-bind` CSS 函数�?CSS 的值链接到动态的响应式数据；Vue会对`v-bind()` 进行特殊处理�?

- 将绑定的变量转换�?CSS 变量
- 在组件根元素上动态设置对应的 CSS 变量�?
...
| 对比         | 选项�?API                             | 组合�?API                |
| ------------ | -------------------------------------- | ------------------------- |
| 逻辑组织方式 | �?**功能类型** 分类                   | �?**业务逻辑** 分类      |
| 代码结构     | data / methods / computed / watch 分开 | 逻辑写在 `setup()` �?    |
...
setup是组件生命周期中最早执行的函数�?`beforeCreate` 钩子之前执行（组件实例尚未创建），因�?`setup` 内部�?`this` 指向 `undefined`，并且每次实例化组件时都会执行一次；
...
export default {
    setup() {
        ...
        function printname() {
            name='bbb'//这里修改name页面不会变化，但是实际上name确实变化�?
            console.log(name);
        };
        return {name,age,printname};
    },
    data(){
        dataName:this.name,//data的this可以读取setup的属性，但是setup不能读取data中属�?
    },
}
</script>
```
...
�?Vue.js 2 中，响应式系统是基于 `Object.defineProperty` 的属性级劫持实现的。Vue 在初始化 `data` 时，会通过 `Observer` 遍历对象的所有属性，并使�?`defineReactive` 对每个属性调�?`Object.defineProperty`，为其定�?`getter` �?`setter`。当组件渲染或计算属性执行时，如果访问了某个响应式属性，就会触发 `getter`，此时通过 `Dep` 进行依赖收集，将当前�?`Watcher` 记录下来；当属性值发生变化时会触�?`setter`，调�?`dep.notify()` 通知所有依赖的 Watcher 执行更新，从而重新渲染组件或执行相应的回�?
...
- defineReactive（非JS原生）：用于为对象的每一个属性通过 Object.defineProperty定义getter �?setter、并创建 Dep（依赖管理器）、并且在 getter / setter 中调�?Dep 方法，触发依赖收集和派发更新

- Dep：一个属性对应一�?Dep，负责存储所有依赖该属性的watcher，并且执行依赖收集（发生在组件渲染，计算属性执行，watch 监听时）和派发更新（通知Watcher执行更新逻辑�?

- Watcher：用于订阅响应式数据（包括渲�?watcher，computed watcher，watch监听），当数据变化时接收通知，并且执行对应的更新逻辑�?
...
| --------------------- | ----------------------------------------------- | ----------------------------- |
| [[Get]]               | Reflect.get(target, key[, receiver])            | 获取属性�?                   |
| [[Set]]               | Reflect.set(target, key, value[, receiver])     | 设置属性�?                   |
| [[HasProperty]]       | Reflect.has(target, key)                        | `key in obj`                  |
| [[Delete]]            | Reflect.deleteProperty(target, key)             | 删除属�?                     |
| [[OwnPropertyKeys]]   | Reflect.ownKeys(target)                         | 获取对象所有属性（�?Symbol�?|
| [[Call]]              | Reflect.apply(target, thisArg, args)            | 函数调用                      |
| [[Construct]]         | Reflect.construct(target, args[, newTarget])    | 构造函数调�?                 |
| [[DefineOwnProperty]] | Reflect.defineProperty(target, key, descriptor) | 定义属�?                     |
...
**Vue3响应式原�?*：基于Proxy（拦截对象操作），WeakMap（存储对象与依赖映射），effect（副作用函数/依赖函数），track / trigger（收集依赖与触发依赖构建）；核心目标：数据变�?�?自动触发依赖 �?更新视图/副作用；

- 响应式对象创建（Proxy）：将对象封装成Proxy对象，并且拦截get/set�?
- 当你调用 `effect(fn)` （模版渲染函数，computed，watch，用户直接调用effect）时，Vue 会先将副作用函数 `fn` 设置�?`activeEffect`（一般全局只有一个activeEffect，嵌套Effect时会使用栈结构，但每次依然只有一个生效的 Effect），然后立即执行它。在执行过程中，如果访问响应式对象的属性，会触�?Proxy �?get 拦截器。拦截器内部会调�?`track(target, key)`，将当前正在执行�?Effect（即 `activeEffect`）注册到 WeakMap（全局共用）的对应对象属性依赖集合里，实现依赖收集。这样，当该属性发生变化时，依赖它�?Effect 会被重新执行。如果访问的属性值本身是对象或数组，Vue 会返回它�?Proxy，从而实现嵌套对象的响应式（懒递归，只有访问到该属性对象时才递归进行Effect流程�?
- 用户修改proxy对象属性时：Proxy �?`set` 拦截器被触发，调�?`Reflect.set` 执行原始赋值，调用`trigger`通知依赖该属性的 effect 执行更新�?
...
```
      ...
      get操作                      set操作
        �?                          �?
        �?                          �?
  Proxy get拦截器触�?          Proxy set拦截器触�?
        �?                          �?
        �?                          �?
  track(target,key)              Reflect.set(target,key,value)
        �?                          �?
        �?                          �?
  判断是否�?activeEffect         赋值成功，原始对象更新
        �?                          �?
        �?                          �?
  �?activeEffect 收集�?          trigger(target,key)
...
- 基本类型无法�?Proxy 代理，ref能将基本类型包装为一个对象，通过 `value` 属性的 getter/setter 拦截读写操作，从而在读取时收集依赖，在修改时触发更新，实现响应式�?
- 对于对象类型 ，Vue 会先�?`RefImpl` 包装数据，再把对象转换为 `reactive Proxy`，通过 `ref` 监听 `.value`，通过 `Proxy` 监听对象属�?
...
reactive 通过Proxy 代理对象类型数据，在读取和修改数据时拦截操作，从而实�?依赖收集（track）和更新触发（trigger�?
...
<template>
  <div class="app">
    ...
    <button @click="b++">Increment</button>//b不能被修�?
  </div>
</template>
```
...
一个手动触�?ref 依赖更新�?API；主要用于配�?`shallowRef` 使用，当内部对象变化�?Vue 无法自动检测时，可以手动通知响应式系统更新视�?
...
const handleClick = () => {
    count.value.count++
    triggerRef(count);//添加triggerRef才会触发页面更新，否则不�?
}
</script>
...
- toRef：把对象的某个属性转换为一�?`ref`，并保持与原对象属性的响应式同步关系；
...
//将person变成响应式赋值给person1，person1修改属性，person也跟着�?
...
#### customRef
 ...
 允许自定�?`ref` 依赖收集与触发更新行为的 API；它接受一个工厂函数作为参数，工厂函数的参数是track和trigger，返回一个包含get和set函数的对象；
...
核心思想：基于依赖自动计�?+ 结果缓存（依赖不变直接返回缓存）+惰性执行（依赖变化`computed` 不会立即执行，只有访问才重新计算�?
...
```
<script setup>
import { reactive, watch } from 'vue' // 导入 reactive �?watch

const state = reactive({ // 创建一�?reactive 响应式对�?
  user: {                // 嵌套对象用于演示 deep 监听
    name: 'Tom',
    age: 20
  },
  keyword: ''           
...
watch(
  ...
  (newValue, oldValue, onCleanup) => { // callback：数据变化时触发；第三个参数用于注册清理函数
  
    const timer = setTimeout(() => { // 创建一个副作用
    
    }, 1000)
    
    onCleanup(() => { // 注册清理函数，在下一�?watch 执行前触�?
      clearTimeout(timer) // 清理上一次的定时�?
    })
  },
  {
    immediate: true, // 组件初始化时立即执行一�?watch 回调
    deep: true,      // 深度监听对象内部属性变化（�?state.user.name�?
    flush: 'post'    // �?DOM 更新完成后再执行回调
  }
)
</script>
```
...
专门用于�?watchEffect �?watch 的副作用函数中注册清理回调。这些清理回调会在下一次副作用重新运行之前或组件卸载时被调用；
...
在模板中�?DOM元素或组件实�?添加 `ref`，可以在 `setup` 中获取对应真实DOM或组件实例；

- 要在组件或元素挂载后（onMounted，nextTick）才能访问其属性（`setup()` 执行�?DOM还没创建，此时ref.value === null）；
- �?`<script setup>` 中子组件默认�?封闭的，父组件无法访问内部属性，必须通过在子组件 `defineExpose()` 显式暴露，才能访问（可改）；
...
�?*单选框**中使用：单项框中要给value设置特定的值，才能让v-model绑定的data变量获得值，否则为null�?

```
�?input type="radio" name="sex" v-model="sex" value="�?>
�?input type="radio" name="sex" v-model="sex" value="�?>
```

�?*多选框**中使用：hobby要设置为数组，当被选中的多选框的value值会push到hobby数组中；
...
�?*选择�?*中使用：data中的变量要绑定到select标签上，被选中的option对应的value值会赋值给data的变�?
...
事件绑定，可以简写为@，后面跟事件名称，事件回调函数；回调函数可以传参数，但要�?event占位符给event对象占位�?
...
**键盘事件修饰�?*：表示只有按下特定键才触发某个事件，可以连续调用表示同时按下键才触发�?
...
```
<button v-on:click="print('aaaaaa',$event)">按钮</button>//$event为事件对象的占位�?

<button @click.prevent="print">按钮</button>

<input type="text" id="root" @keyup.enter.y="print">//同时按下enter和y才触发事�?
```
...
- 不能�?`v-else / v-else-if` 配合�?
- 不能作用�?`<template>`，必须绑定在真正的DOM元素上；
...
- �?`v-if` �?`v-for` 同时存在于同一个元素上时，`v-for` 的优先级更高，这意味着：循环会先执行，再在每次循环中执�?`v-if` 判断（v-if判断的条件，需要v-for产生）（不推荐元素同时使�?v-if �?v-for，而是提前filter在循环）
...
//数字范围
    <ul>
        <li v-for="k in n" :key="k">{{k}}</li>//展示10个内容为�?�?0的li
    </ul>
</div>
```
...
<div v-html="message"></div>//会执行onclick事件，即会执行JS脚本;
<script>
...
一个属性，当Vue实例对象建立完毕后就会删除，当浏览器因其他原因延迟解析script导致Vue实例对象延迟建立，而引发不能渲染DOM时，使用v-cloak配合使用CSS的属性选择器，可以使DOM元素暂时不显示在页面�?
...
<template>
    ...
    <div v-once>Home once {{ count }}</div>//永远等于0
</template>
```
...
编译器宏函数是专�?`<script setup>` 语法糖设计的编译阶段特殊语法，在编译阶段�?Vue 编译器特殊处理的函数标记，它们不会在运行时存在，而是在编译时被转换成普�?JavaScript 代码，简�?`<script setup>` 中组件的配置（props、emits、选项等）�?

Vue SFC 编译器（`@vue/compiler-sfc`）会自动识别，无需手动导入宏函数；并且宏函数只能在 `<script setup>` 的顶层使用；

#### defineProps

获得组件接收的属性，并返回一个响应式�?props 对象；接受一个对象作为参数，并返回一个含有传入值的proxy对象，通过这个proxy对象可以访问到传入值（传入值是只读并具有响应式的）；在父组件中传递变量的名称要和子组件的defineProps中的一样；
...
`<script setup>` 中子组件的内部变量和方法默认是封闭的，父组件无法通过 ref 获取，defineExpose 用于显式暴露需要对外提供的属�?和方法；暴露的属性支持响应式�?
...
const handleClick = () => {
  ...
  childRef.value.increment() // 调用子组件方�?
}
</script>
<template>
  <Child ref="childRef" />
  <button @click="handleClick">调用子组件方�?/button>
</template>
```

#### defineOptions

替代传统选项�?API 的顶层选项（如 `name`、`inheritAttrs`、`components` 等）�?
...
用于在子组件中声明一个可通过 `v-model` 绑定�?prop，并自动生成对应�?`update:xxx` 事件，从而实现父子组件之间的双向数据同步；（本质是`prop + emit('update:xxx')`实现双向绑定的语法糖）；
...
用于为插槽提供类型信息，�?IDE 能正确提�?slot 的参数结�?
...
用于将组件模板中的部�?DOM 节点渲染到页面中指定的目标位置；

- 虽然这些 DOM 在实际页面结构中被移动到了其他位置，但在逻辑上它们仍然属于原来的组件树，因此组件之间�?`props`、`emit`、`provide/inject` 等关系不会受到影�?
...
- teleport标签属性：
  ...
  2. disabled：控�?是否启用 teleport，如果为 `true`，就不会进行传送，而是在原位置渲染�?
  3. defer：延�?teleport 挂载，等目标节点出现再挂载，解决目标节点晚于组件渲染的问题；
...
本质是通过 `resolveDynamicComponent` 解析组件类型并生成对应的 VNode，当组件类型变化�?Vue 会卸载旧组件并挂载新组件，如果结�?`keep-alive` 则可以实现组件缓存；

- 通过is属性来决定要渲染哪个组件，并且可以添加其他属性作为props参数（但是需要统一props防止多个组件不兼容）�?
- 组件切换时会销毁旧组件（触发完整生命周期），可以使用keep-alive包裹保留状态；
- 如果多个组件结构类似，Vue 可能�?复用 DOM，出现状态错乱，可以添加key属性；
...
import { ref } from 'vue'
let which = ref(Son)
function toggle() {//切换which的值为Son1
    which.value =Son1
}
</script>

<template>
    <component :is="which" :key="current"></component>//渲染哪个组件根据which来决�?
...
<style scoped>
    .Name-enter-active{
        transition: all 2s;//为整个插入过程添加过渡，过渡所有样式，时间�?s
    }
    .Name-enter-from{//插入过渡开始时样式
        font-size: 10px;
        color:aqua;
    }
    .Name-enter-to{//插入过渡结束时样�?
        font-size: 30px;
        color:black;
    }
...
v-for的列表过渡效果，transition只能设置单个元素，而transition-group可以设置多个元素；每个列表元素都要设�?key否则Vue无法跟踪�?
...
- 如果所有组件都 同步加载，构建工具会把组件全部打包到一�?`bundle.js` 中，可能导致JS 解析时间长，首屏加载慢；

- 使用异步组件后，构建工具会进行代码分割，生成主包`bundle.js`和按需加载的模块：`chunk-A.js`、`chunk-B.js`，只有在 真正渲染该组件时，浏览器才会去加载对应的 `chunk` 文件�?

#### Suspense

用于统一管理组件树中的异步组件（由defineAsyncComponent或`<script setup>` 中使�?`await`的组件），并在异步完成前显示 fallback（加载状态）�?

```
渲染 Suspense
�?
发现 AsyncComponent 未加�?
�?
显示 fallback
�?
AsyncComponent 加载完成
...
`defineAsyncComponent` �?Vue3 中用于生成异步组件的 API

```
import { defineAsyncComponent } from 'vue'

const AsyncComp = defineAsyncComponent({
  loader: () => import('./MyComponent.vue'), // 异步加载组件，返�?Promise，组件在渲染时才会动态加�?
  loadingComponent: LoadingComponent, // 组件加载过程中显示的占位组件
  errorComponent: ErrorComponent, // 组件加载失败或超时时显示的组�?
  delay: 200, // 延迟 200ms 再显�?loadingComponent，避免组件很快加载完成时出现闪烁
  timeout: 3000, // 最长等�?3000ms，超过时间仍未加载成功则视为失败
...
const app = createApp(App)
app.config.globalProperties.pai=3.14;//全局都能直接用pai这个变量，{{pai}}
app.mount('#app')
```
...
局部指令：�?`<script setup>` 中声明以 `v` 开头的变量，Vue 会自动将其注册为当前组件的指令，例如 `vFocus` 对应模板中的 `v-focus`�?

app.directive参数�?

- 第一个参数为指令名称（不需�?`v-` 前缀，使用时要）�?
...
<template>
  <div class="app">
    <input type="text" v-my-focus:100="a" />//binding.arg�?100"，binding.value为a;
  </div>
</template>
```
...
//main.js
import pugin from './plugins/pugin';
const app = createApp(App);

app.use(pugin,{name:"aaa",age:23});
app.mount('#app');
```
...
//每次触发increment事件时，a.value加一引发DOM更新，又调用nextTick来获得新的a.value的�?
const increment = () => {
    a.value++;
    nextTick(() => {
...
nextTick(() => {//如果把nextTick放在这里，那它只会执行一次输�?
  console.log('Updated value:', a.value);
});
</script>
...
用于跨层级组件通信的一�?API，本质上解决的是祖先组件 �?任意深度后代组件的数据传递问题，避免层层 `props` 透传�?
...
- provide的键优先使用Symbol类型（并且导出），`inject` 是向上查找最近匹�?key，防止冲突；
...
用于�?`setup` 阶段（在setup周期之前创建）获取当前组件实例；
...
用来生成在应用内稳定且唯一�?ID，主要用于：SSR（服务端渲染�? 客户端水合一致性和无障碍（a11y）属性绑定（label / input）；
...
- 当子组件需要多个插槽位置时，可以使�?`name` 为插槽命名，父组件通过 `v-slot:name`（或 `#name`）向对应的插槽传入内容（需要使用`<template>`）；
...
<template>
  <div>
    <Son >
      <template v-slot:s1>//插入名为s1的slot标签的内�?
        <h1>s1插槽</h1>
      </template>
      
      <template #s2>//插入名为s2的slot标签的内�?
        <h1>s2插槽</h1>
      </template>
    </Son>
  </div>
...
- 子组件通过 `<slot>` �?props 向父组件暴露数据，父组件�?`template` 中接收并使用这些数据进行渲染�?
...
用于�?`setup` 里访问插槽对象的API（组合式API使用this.$slots访问），相当于`getCurrentInstance().slots`�?
