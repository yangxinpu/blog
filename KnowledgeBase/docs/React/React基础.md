...
react包： React 核心库，定义�?UI 的抽象模�?

- JSX �?`React.createElement`（生成虚�?DOM / React Element�?
- 组件模型（函数组�?/ 类组件）
- Hooks（`useState`、`useEffect` 等）

React渲染器： �?React 描述�?UI（React Element / 虚拟 DOM）转化为具体平台视图的执行层实现，负责构建Fiber 树，新旧 Fiber �?diff，执行响应更新操作；

- Web �?`react-dom`
- Native �?`react-native`
- SSR �?`react-dom/server`

createRoot方法：接收一�?DOM作为参数，创建的是一个Fiber Root（根 Fiber 节点�?
...
用类�?HTML 的写法，描述 UI 结构，最终会被编译成 JavaScript；（如`const element = <h1>Hello</h1>`会被编译成`const element = React.createElement('h1', null, 'Hello')`�?

-  必须有一个根节点，可以用Fragment（不生成额外 DOM）；
- 标签名以小写字母开�?�?视为原生 DOM 标签，标签名以大写字母开�?�?视为 React 组件（变量引用）�?
...
SCSS 负责增强 CSS 编写能力，CSS Modules 负责作用域隔离（类名hash，生成对应hash的JS类名映射对象），最终由构建工具把它们编译成浏览器可执行的普�?CSS + 映射对象
...
将CSS模块化，通过对象键来使用，样式文件名必须�?module.scss�?
...
一个在 React 生态中�?CSS-in-JS 方案，它允许你在 JavaScript/TypeScript 中直接写 CSS，并将样式绑定到组件上；

- styled-components本质上是调用一个函数（标签函数），将模板字符串中的 CSS 代码作为参数传入，并为每�?style 组件生成唯一的哈希类名（�?`sc-bdVaJa`）；styled-components会在运行时，创建一�?`<style>` 标签（或复用已有的）插入�?`<head>` 中，将生成的类名与对应的 CSS 规则写入 `<style>` 标签�?

安装：`npm install styled-components`�?

##### 基本语法

- `styled-components` 支持在模板字符串中使用插值表达式 `${}`，插值可以是：JS变量，函数（接收 `props`，返�?CSS 值）

- `&` 表示当前组件生成�?class

- `styled(已有组件)` 用于扩展已有样式，类�?CSS 的继�?+ 覆盖�?
...
用于向整个组件树注入 theme 对象，使所�?styled 组件可以通过 props.theme 访问统一设计变量�?
...
用于声明全局 CSS，并在组件挂载时自动注入�?`<style>` 标签�?
...
函数本身作为render函数，接�?props，返回值为渲染的组件；
...
- Hook链的头节点会被保存在当前组件 Fiber �?`memoizedState` 上；并且每个 Hook 节点保存各自的状态信息，比如：当前状态值，更新队列，副作用依赖，下一�?Hook 节点指针�?
- React �?Hook 的状态复用依赖的是调用顺序，而不是变量名，React 在函数组件重新渲染时，会按照 Hook 的调用顺序，依次从旧�?Hook 链表中取出对应节点，并在此基础上构建新�?Hook 链表（每次更新都会创建新的Hook链），从而实现状态的复用（因此，Hooks 必须在组件顶层按固定顺序调用，不能写在循环、条件或嵌套函数中，否则会导致顺序错乱）�?
...
- 状态更新函数：用于�?React 提交状态变更请求，从而触发组件重新渲染；它既可以接收一个新值，也可以接收一个以旧状态为参数的回调函数；调用状态更新函数时不会立即修改状态，而是�?React 在当前更新周期中对多次调用进行批量合并后再统一更新并渲染；因此，状态更新函数的调用是同步执行的，但状态本身的更新是延迟生效的，表现为类似异步的行为；

�?React中，多个 setState（包括嵌套调用）都会被放入对�?Hook 的更新队列中，并在同一个批处理上下文中统一执行，从而只触发一次渲染；

React 18 引入了自动批处理机制，使得不仅在事件处理函数中，在异步回调（�?setTimeout、Promise）中也会进行批处理，只有当更新跨越不同的任务边界（如 await 之后），才会产生多次渲染�?
...
副作用函数：函数执行过程中，除了返回 UI或计算值外，对外部环境产生的额外影响或依赖外部环境的操作，如操�?DOM ，数据请求，事件监听，修改localStorage/sessionStorage，打印日志（`console.log`）等；不应该在副作用函数中使用setState，否则可能循环渲�?

```
render �?commit �?cleanup �?effect
```

- 在组件挂载后（即组件渲染�?DOM 并完成浏览器绘制后）异步执行副作用函数；
- 当依赖变化时（依赖不变则不会执行副作用函数），组件会先重新渲染（计算出变化的地方）并提交更新�?
- 然后在执行新的副作用函数之前先执行上一次的清理函数�?
- 组件卸载时会执行所�?`useEffect` 的清理函数，但不会再执行副作用函数；
...
解决方式包括正确声明依赖数组以让 effect 重新执行（旧的异步会被清理）、使�?`useRef` 持有最新值，或使�?`useEffectEvent`来避免闭包带来的旧值问�?
...
`useEffectEvent` 用于解决 `useEffect` 中由于闭包导致的旧状态问题，它可以让副作用中的回调在执行时始终访问到最新的状态和 props，同时不会因为这些值的变化而触�?`useEffect` 重新执行，从而避免将其加入依赖数组带来的重复执行问题�?

`useEffectEvent` 的本质可以理解为 React 内部封装了一种类�?`useRef` 的机制：在组件每次渲染时，将最新的状态和 props 保存到内部的引用容器中，并返回一个引用稳定的回调函数；该回调函数在执行时会从这个容器中读取最新的值，从而既保证了回调引用不变，又避免了闭包捕获旧状态的问题�?
...
`useLayoutEffect` 用于**同步**处理与布局相关的副作用，其用法�?`useEffect` 基本一致，但执行时机是�?DOM 更新完成后、浏览器绘制之前同步执行�?
...
function ClosureFix() {
  const [count, setCount] = useState(0);
  // �?ref 存储最�?count（每次渲染同步更新）
  const countRef = useRef(count);

  // 每次 count 变化时，同步�?ref.current
  useEffect(() => {
  ...
  useEffect(() => {
    // 异步任务�?秒后打印最�?count
    setTimeout(() => {
      console.log('最�?count�?, countRef.current); // 访问 ref，拿到最新�?
    }, 1000);
  }, []); 
...
- Consumer 消费组件：`useContext` 在渲染时会直接读取当�?Context 的当前值（由最近的 Provider 在渲染过程中设置）；
- _currentValue属性：挂在 Context 对象上的内部字段，用于存储当前渲染环境下�?Context 的值；

原理�?

- 当子组件调用 `useContext(Context)` 时，React 会在当前组件的渲染过程中，从 Context 栈中读取来自最近的上层 `Provider`�?Context 当前值；

-  同时，React 会在当前消费者对应的 Fiber 上记录它对该 Context 的依赖�?

-  当某�?`Provider` �?`value` 发生变化时，React 会从这个 Provider 的子树开始传播更新，找到依赖�?Context 的消费�?Fiber，并调度这些组件重新渲染�?
...
- 当value是对象时，在每次父组件渲染都会生成新的引用，导致所有消费子组件重新渲染，可以使�?useMemo缓存 value，仅当依赖变化时才更新引用；
...
解决：函数组件每次渲染时，内部定义的函数都会被重新创建（生成新的引用）如果这个函数作�?props 传递给子组件，即使函数逻辑完全没变，子组件也会认为props 变了（因为引用变了），从而触发子组件的重渲染，造成性能浪费�?
...
- 第一个参数为需要延迟更新渲染的原始值（一般是状�?props，可以是普通值），返回带有延迟渲染的值（派生值是只读的，修改原始值会延迟同步到派生值）�?
- 第二个参数为配置对象，仅支持一个属性timeoutMs（设置最大延迟时间，即使主线程一直忙碌，超过该时间后，非紧�?UI 也会强制更新，默认值为无）
...
const FilteredList = memo(({ deferredQuery, items }) => {
  //�?useMemo 缓存过滤结果：仅�?deferredQuery 变化时才重新过滤
  const filteredItems = useMemo(() => {
    return items.filter(item => item.name.includes(deferredQuery));
  }, [deferredQuery, items]); // 依赖变化才重新计�?
  return (
    <ul>
...
function App() {
  ...
  const deferredQuery = useDeferredValue(query, {
    timeoutMs: 500, // 最多延�?00ms，避免用户长时间看不到结�?
  });
...
用于React的并发渲染优化：用于标记非紧急的状态更新操作，�?React 优先处理用户交互（如输入、点击）等紧急操作，避免非紧急更新阻�?UI 导致卡顿，从而提升应用响应性；
...
- useTransition：包裹非紧急更新的操作，自带isPending标记过度状�?
...
function SearchList() {
  const [inputValue, setInputValue] = useState(''); // 紧急更新：输入框�?
  const [filteredList, setFilteredList] = useState([]); // 非紧急更新：过滤后的列表
  const [isPending, startTransition] = useTransition({
    timeoutMs: 300, // 300ms 内完成，否则显示加载状�?
  });
  ...
  const handleInputChange = (e) => {
    ...
    setInputValue(value);
    //非紧急更新：�?startTransition 包裹，避免阻塞输�?
    startTransition(() => {
      // 过滤逻辑
...
React 18 提供的用于安全订阅外部状态源�?Hook，保证在并发渲染下不会发生数据撕裂（不一致）

用于保持外部状态（�?Redux、localStorage、navigator.onLine ）在并发渲染模式下数据一致性；

参数和返回值：

- 第一个参数为订阅函数：接收一�?callback作为参数，当外部状态发生变化时（需要通过特定方法监听），需要手动调用这个回调，以此通知React外部状态已更新，调用callback后React会触发重新渲染；
...
�?commit 阶段前，React 会再次调�?getSnapshot，如果发现快照发生变化，会放弃当前渲染并重新执行，从而保�?UI 与外部状态始终一致，避免 tearing
...
`use` �?React 提供的一个用于在 render 阶段直接读取异步资源（Promise / context）的 API，它通过抛出 Promise + Suspense实现同步写法的异步数据读取，简化useEffect+useState的异步读取逻辑�?

- 在组件内部直接创�?Promise，会导致组件每次渲染时都生成一个新�?Promise 实例，由于这�?Promise 没有被缓存，React 目前不支持在组件渲染过程中对这类未缓存的 Promise 使用 `use` 进行解析�?
- `use` 会在渲染阶段同步执行，如果数据尚未就绪，它会抛出一�?`Promise` �?React 等待；如果数据已经可用，就直接返回读取结果；
...
需要继�?React.Component，必须定义render() 方法返回React组件的虚拟DOM�?
...
用于表示组件内部管理的数据，�?state的数据发生变化时，React会重新调用组件的渲染方法（函数式组件重新执行函数，类组件重新调用 render()），重新渲染依赖该state的地方；
...
一种组件间共享逻辑的设计模式，父组件通过接收一个返回子组件的函数作�?props（通常命名�?`render` 或利�?`children`），将自身的状态或逻辑传递给这个函数；相当于Vue的插槽； 
...
React.createRef用于创建一个存储被ref所标识的节点的容器，该对象有一�?current 属性，用于存储dom元素�?
...
内联回调形式在每次组件重新渲染时，先调用旧回调（传入 `null`），再调用新回调（传�?DOM 元素），即每次渲染都会创建新的函数，并额外触�?2 次调用；
...
React 的事件处理机制是在原�?DOM 事件基础上封装的一套合成事件（SyntheticEvent）系统；

- React 并不是直接把事件绑定到真�?DOM 上，而是通过事件委托（收集路径上的回调，按顺序执行）统一绑定在根节点（React 17 之前�?`document`，之后是 root 容器）；
- React 事件默认是冒泡阶段触发的（类�?DOM �?bubble），如果需要捕获阶段，可以使用 `onClickCapture` 这类写法�?
-  React 会对原生 DOM 事件封装一个SyntheticEvent 对象，用于提供跨浏览器一致的事件 API：在每次事件触发时都会创建SyntheticEvent 对象，传入对应回调函数；

回调函数写法�?

- 直接传入函数引用：自动接�?`SyntheticEvent`�?不能直接传自定义参数
...
SyntheticEvent �?React 对原�?DOM 事件的封装对象，在事件触发时创建，并且传给对应回调函数；

-  跨浏览器统一：不同浏览器事件行为不一致，React 做统一封装�?
- 早期 React 为了性能使用事件池复用机制，会在回调后清空事件对象，需要通过 `event.persist()` 保留（会有异步问题）；�?React 17 之后取消了该机制，事件对象不再复用，可以在异步场景中直接使用�?
...
推荐使用受控组件：因为受控组件符�?React 的“数据驱�?UI”思想，使 UI 完全�?state 决定，从而保证一致性、可预测性和可控性；
...
非受控组件：表单数据�?DOM 自身控制，React 不直接管理状态，而是通过 `ref` 手动获取 DOM 元素的值；
...
class MyComponent extends React.Component {
  ...
  render() {
    return <div>{this.state.data ? this.state.data : "加载�?.."}</div>;
  }
}
```
...
lazy�?接受一个动态的 `import()` 表达式作为参数（只能用于默认导出的组件），并返回一个新�?React 组件；当该组件需要渲染时，React 才会异步加载该组件的代码，从而实现懒加载�?
...
`ErrorBoundary` 组件本身并不�?React 的内置组件；它是一个由开发者自己实现的组件，用来捕获子组件中的错误并处理它们，防止整个应用崩溃

- 错误边界传统上是一个类组件，因为它依赖于生命周期方�?`componentDidCatch` �?`getDerivedStateFromError`
...
- 不能捕获以下情况的错误：只能使用`try-catch`单独处理（可以在catch中再次抛出错误给边界处理）；
  ...
  - 生命周期外的错误：只能捕获渲染阶�?�?生命周期方法*中发生的错误。它不捕获例如构造函数或静态方法中的错误；
...
class ErrorBoundary extends Component {
  ...
  render() {
    ...
    return this.props.children; // 正常渲染子组�?
  }
}
...
用于在不添加额外DOM 节点的情况下，让组件返回多个同级元素；它解决�?React 组件中必须有一个单一根元�?的限制；现在推荐使用`<>...</>`空标签；
...
React 中某些生命周期方法（�?`componentWillMount`、`componentWillReceiveProps`、`componentWillUpdate`）由于设计缺陷，容易导致 bug，已被标记为不安全`<StrictMode>` 会在检测到这些方法时在控制台抛出警告；

#### PureComponent（类�?

  React提供的一个类组件基类（继承自 React.Component）；它的核心特性是自动实现�?`shouldComponentUpdate` 方法，通过�?`props` �?`state` 进行浅比较来决定是否需要重新渲染组件，从而避免不必要的渲染操作；类似函数式组件的memo�?
...
class Parent extends React.Component {
  ...
  render() {
    return (
      <div>
      //点击按钮时，父组件因 count 变化重新渲染，但 UserCard 接收�?props（name �?age）没有变化，因此 UserCard 不会重新渲染
