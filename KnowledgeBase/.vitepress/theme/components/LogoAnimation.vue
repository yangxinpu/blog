...
const mouseX = ref(0)
const mouseY = ref(0)
const particles = ref<Array<{ id: number; x: number; y: number; size: number; color: string; delay: number }>>([])
const containerRef = ref<HTMLElement | null>(null)

const colors = ['#00d5c4', '#19fac6', '#00b8a9', '#00ffcc', '#4FC08D', '#61DAFB']
...
.logo-glow {
  position: absolute;
  inset: -20px;
  background: radial-gradient(circle, rgba(0, 213, 196, 0.3) 0%, transparent 70%);
  border-radius: 50%;
...
@keyframes particle-float {
  ...
  100% {
    transform: translate(-50%, -50%) translate(calc(var(--x) * 1.5), calc(var(--y) * 1.5)) scale(0);
    opacity: 0;
  }
}
...
.dot:nth-child(3) { bottom: 20%; left: 10%; }
.dot:nth-child(4) { bottom: 10%; right: 30%; }
.dot:nth-child(5) { top: 50%; left: 5%; }
.dot:nth-child(6) { top: 5%; right: 50%; }
