...
const ManifestoSection: React.FC<ManifestoSectionProps> = ({ theme, word }) => {
  ...
  return (
    <section ref={containerRef} className={`relative min-h-screen flex items-center justify-center py-32 px-4 md:px-12 lg:px-24 overflow-hidden ${theme === 'dark' ? 'text-white' : 'text-nl-black'}`}>
          ...
          {structure.map((lineWords, lineIndex) => (
            <div key={lineIndex} className="flex flex-wrap justify-center gap-x-3 gap-y-2 md:gap-x-4 md:gap-y-3 leading-tight">
              {lineWords.map((word, wordIndex) => {
                ...
                return <InteractiveWord key={`${lineIndex}-${wordIndex}`} word={word} progress={scrollYProgress} range={[start, end + 0.1]} theme={theme} />;
              })}
            </div>
          ))}
        </div>
...
const InteractiveWord: React.FC<WordProps> = ({ word, progress, range, theme }) => {
  ...
  const color = useTransform(progress, range, theme === 'dark' ? ['#334155', '#ffffff'] : ['#cbd5e1', '#0f0f0e']);
  ...
  return (
    <motion.span
      ...
      whileHover={{
        ...
        textShadow: theme === 'dark' ? `0 0 20px ${COLORS.neonGreen}, 0 0 10px ${COLORS.neonGreen}` : `0 0 20px ${COLORS.lightGreen}`,
