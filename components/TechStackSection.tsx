    ...
    <section ref={targetRef} className="relative h-[400vh] z-20">
      <div className="sticky top-0 flex h-screen items-center overflow-hidden perspective-1000">
        <div className="absolute top-24 md:top-32 left-8 md:left-20 z-50 pointer-events-none">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }} viewport={{ once: true }} className="mb-4">
            <h2 className={`text-5xl md:text-7xl font-black drop-shadow-2xl ${theme === 'dark' ? 'text-nl-neon' : 'text-nl-dark'}`}>{title}</h2>
          </motion.div>
          <motion.p
  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        style={{
          background: useTransform([glareX, glareY], ([x, y]) => `radial-gradient(circle at ${x} ${y}, rgba(255,255,255,0.3) 0%, transparent 80%)`),
        }}
      />
      ...
      <div className="relative h-full flex flex-col p-10 z-10" style={{ transform: 'translateZ(50px)' }}>
        ...
        <div className="flex-grow">
          <p className={`text-xl leading-relaxed font-medium ${isDark ? 'text-gray-300' : 'text-gray-600'}`}>{tech.description}</p>
        </div>
